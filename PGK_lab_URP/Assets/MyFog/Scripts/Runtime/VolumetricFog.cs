...
public sealed class VolumetricFog : VolumeComponent, IPostProcessComponent
{
    ...
    [Tooltip("Allows you to control whether Buto will additively blend fog masks or exclusively render fog masks.")]
    ...
    // Stylized
    [Tooltip("A texture that can be used to give the fog an emission value and to manipulate the fog color in lit and shadowed areas. Typically used for stylized effects.")]
    public Texture2DParameter colorRamp = new Texture2DParameter(null);

    [Tooltip("Controls the amount of influence the color ramp exerts over the base physical results. A value of 0 means the Color Ramp is completely ignored. A value of 1 means that we exclusively refer to the Color Ramp.")]
