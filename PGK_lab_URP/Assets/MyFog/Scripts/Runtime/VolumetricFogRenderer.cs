...
public class VolumetricFogRenderer : ScriptableRendererFeature
{
    ...
    class RenderFogPass : ScriptableRenderPass
    {
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            void SetMaterialParameters()
            {
                ...
                void SetVolumeData()
                {
                    Vector4[] positionsAndRadius = new Vector4[Common._MAXVOLUMECOUNT];
                    float[] intensities = new float[Common._MAXVOLUMECOUNT];

                    if (FogDensityMask.FogVolumes.Count > Common._MAXVOLUMECOUNT)
                        FogDensityMask.SortByDistance(camera.transform.position);

                    int volumeCount = Mathf.Min(FogDensityMask.FogVolumes.Count, Common._MAXVOLUMECOUNT);
